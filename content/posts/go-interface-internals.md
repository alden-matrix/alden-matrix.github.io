...
**一般来说，一个类型的方法尽量统一用值接收者或指针接收者。** 混用会导致 `T` 和 `*T` 的方法集不一致，容易搞不清楚谁实现了哪个接口。不过标准库也有合理混用的例子（比如 `time.Time` 的大部分方法是值接收者，但 `GobDecode` 等需要修改状态的方法用指针接收者），关键是搞清楚你的类型主要以值还是指针的形式使用。
